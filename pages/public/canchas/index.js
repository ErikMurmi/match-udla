...
export default function Canchas(props) {
  ...
  const handleHoraReserva = (e) => {
    ...
    if (checked) {
      if (horarioReserva.length == 2) {
        alert("Solo se puede seleccionar máximo dos horas");
        e.target.checked = false;
        return;
      }
      const primerHorario = horarioReserva[0];

      if (horarioReserva.length == 1 && primerHorario.fin !== horario.inicio) {
        alert("Solo se puede seleccionar horarios consecutivos");
        e.target.checked = false;
        return;
      }
      ...
      let h = Array.from(
        horarioReserva.filter((item) => item.inicio !== horario.inicio)
      );
      setHorarioReserva(h);
    }
  };
  ...
  const handleSubmitReserva = async (form) => {
    ...
    if (!horarioReserva.length > 0) {
      alert("Se debe seleccionar un hora");
      return;
    }
  ...
  return (
    <div className="home">
      ...
      <div className="mid-dis-container">
        ...
        <h4>Seleccionados</h4>
        <div className="horarios-container">
          {horarioReserva.map((horario, index) => (
            <div className="horario-reserva" key={index}>
              <label htmlFor="horario">{`${horario.inicio}-${horario.fin}`}</label>
        ...
        <button
          ...
          onClick={(e) => handleSubmitReserva(e)}
        >
          Reservar
        </button>
      </div>
